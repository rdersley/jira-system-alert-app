...
function launcher(){
  if (byId('samSetupLauncher')) return;
...
async function loadData(){ state.data = await invoke('getAdminData'); }
function ready(){ const d=state.data; return Boolean(d?.setupStatus?.jira && d?.setupStatus?.email && Number(d?.setupStatus?.contacts||0)>0); }
function pill(ok,label){ return `<span class="sam-w-status ${ok?'ok':'pending'}">${ok?'✓':'!'} ${esc(label)}</span>`; }
...
function collectPriorities(){ return [...document.querySelectorAll('.sam-w-priority')].map(r=>({name:r.querySelector('.sam-w-priority-name')?.value?.trim()||'',label:r.querySelector('.sam-w-priority-label')?.value?.trim()||'',color:r.querySelector('.sam-w-priority-color')?.value||'#0C66E4'})).filter(x=>x.name); }
...
function body(){
  ...
  if(state.step===1) return `<div class="sam-w-step"><h2>1. Jira setup</h2><p>Choose the Jira project, client field and priorities that can use System Alert.</p><label>Jira project<select id="samWProject">${projectOptions(s.allowedProjectKey||'')}</select></label><label>Client / Customer field<select id="samWClientField">${fieldOptions(s.clientFieldId||'')}</select></label><div class="sam-w-section-label">Alert priorities</div><div>${priorityRows()}</div></div>`;
  ...
  if(state.step===3) return `<div class="sam-w-step"><h2>3. Email</h2><p>Configure Microsoft 365 or SendGrid in the existing Communication Providers screen.</p><div class="sam-w-provider-state">${pill(p.email?.configured,p.email?.configured?`${p.email.provider} ready`:'Email needs setup')}<div>${esc(p.email?.from||'No sender configured')}</div></div><button id="samWOpenProviders" class="sam-w-inline-btn" type="button">Open Communication Providers</button></div>`;
...
function render(){
  ...
  root.innerHTML=`<div class="sam-w-backdrop"><div class="sam-w-dialog"><header><div><span class="sam-w-kicker">SYSTEM ALERT MANAGER · v3.9.6</span><h1>Setup guide</h1></div><button id="samWClose" type="button">×</button></header><div class="sam-w-progress">${steps.map((x,i)=>`<button type="button" data-w-step="${i}" class="${i===state.step?'active':''} ${i<state.step?'done':''}"><span>${i===0?'•':i}</span>${esc(x)}</button>`).join('')}</div>${state.error?`<div class="sam-w-alert error">${esc(state.error)}</div>`:''}${state.message?`<div class="sam-w-alert success">${esc(state.message)}</div>`:''}<main>${body()}</main><footer><button id="samWBack" class="sam-w-secondary" type="button" ${state.step===0?'disabled':''}>Back</button><div><button id="samWExit" class="sam-w-secondary" type="button">Continue to admin</button>${state.step<steps.length-1?'<button id="samWNext" class="sam-w-primary" type="button">Save & continue</button>':'<button id="samWFinish" class="sam-w-primary" type="button">Finish setup</button>'}</div></footer></div></div>`;
  bind();
}
...
async function openWizard(){
  ...
  let root=byId('samSetupRoot'); if(!root){root=document.createElement('div');root.id='samSetupRoot';document.body.appendChild(root);}
...
function bind(){
  ...
  if(byId('samWNext'))byId('samWNext').onclick=()=>act(async()=>{
    if(state.step===1){ const pri=collectPriorities(); if(!value('samWProject')||!value('samWClientField'))throw new Error('Choose a Jira project and Client / Customer field.'); if(!pri.length)throw new Error('Configure at least one priority.'); await saveCore({allowedProjectKey:value('samWProject'),clientFieldId:value('samWClientField'),priorityConfigs:pri}); }
  ...
  if(byId('samWSaveContact'))byId('samWSaveContact').onclick=()=>act(async()=>{
    ...
    await invoke('saveContact',{clientOptionId:value('samWContactClient'),name:value('samWContactName'),email:value('samWContactEmail'),mobile:value('samWContactMobile'),priorities:[...document.querySelectorAll('.samWContactPriority:checked')].map(x=>x.value),emailAlerts:checked('samWEmailAlerts'),smsAlerts:checked('samWSmsAlerts'),monthlyTestAlerts:false});
    state.message='Contact saved.'; await loadData();
  });
  if(byId('samWSendTest'))byId('samWSendTest').onclick=()=>act(async()=>{ const recipient=value('samWTestEmail'); if(!recipient)throw new Error('Enter a test email recipient.'); await invoke('testEmailProvider',{recipient}); state.message=`Test email sent to ${recipient}.`; await loadData(); });
}
